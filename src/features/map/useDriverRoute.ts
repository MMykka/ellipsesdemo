...
export interface DriverRoute {
  ...
  /** Set when one extra trip is folded into `stops` as a preview only — see buildDriverStops. */
...
export function useDriverRoute(driverId: string | undefined): DriverRoute | undefined {
  ...
  return useMemo(() => {
    ...
    const stops = buildDriverStops(trips, driverId, manualOrder, previewTripId)
    ...
    return {
      ...
      previewTripId: sequenced.some((s) => s.isPreview) ? previewTripId : undefined,
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- shiftStart intentionally uses "now" at render time, not a dependency
  }, [driverId, driver, trips, manualOrder, setOrder, previewTripId])
}
