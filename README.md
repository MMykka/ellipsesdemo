...
An internal dispatch tool for planning non-emergency medical transport (NEMT) trips: import a daily Excel export into trip cards, then use a map + dispatch table to check whether a driver can feasibly take a trip and assign it. Everything runs locally and offline except two deliberate, narrowly-scoped exceptions — address geocoding (bare address text only) and an optional road-route preview on the map (coordinates only) — see Phase 2 and Phase 3 below.
...
- **Phase 3 — Map + driver routes.** The Routing page renders an offline vector map (MapLibre + PMTiles — see `docs/OFFLINE_SETUP.md`) showing a selected driver's assigned stops as numbered pickup (green) / dropoff (red) pins plus a black "G" garage marker. Assign a trip to a driver from its card on the Trips page to see it appear here. Click a stop's number in the dispatch table to grow its pin (any number at once — handy when stops overlap), or click its name to draw that leg's travel path in blue: instantly as a straight-line estimate, then upgraded in place to the actual road path once OSRM's free public routing demo (`router.project-osrm.org` — no API key, no account, coordinates only, never trip/member details) resolves it. That lookup is a map visual aid only — it never affects the ETA/feasibility numbers, which stay straight-line-based (see Phase 4).
